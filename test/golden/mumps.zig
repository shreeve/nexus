...
pub const BaseParser = struct {
    ...
    fn executeAction(self: *BaseParser, ruleId: u16, pass: []Sexp) Sexp {
        return switch (ruleId) {
            ...
            337 => .{ .list = &[_]Sexp{} },
            338 => blk: { var out = self.extendList(pass[1]) catch break :blk .nil; while (out.items.len > 0 and out.items[out.items.len - 1] == .nil) _ = out.pop(); break :blk self.keepList(&out); },
            339 => self.list(pass),
            340 => self.sexp(.@"*", &.{}),
            341 => self.list(pass),
            342 => self.list(pass),
            343 => blk: { var out = self.extendList(pass[1]) catch break :blk .nil; while (out.items.len > 0 and out.items[out.items.len - 1] == .nil) _ = out.pop(); break :blk self.keepList(&out); },
            344 => self.list(pass),
