...
pub const BaseSexer = struct {
    ...
    fn tryIdentAsFn(self: *BaseSexer, token: Token, text: []const u8) ?u16 {
        _ = token;
        const state = self.stateStack.getLast();
    ...
    fn tryIdentAsIsv(self: *BaseSexer, token: Token, text: []const u8) ?u16 {
        _ = token;
        const state = self.stateStack.getLast();
    ...
    fn tryIdentAsSsvn(self: *BaseSexer, token: Token, text: []const u8) ?u16 {
        _ = token;
        const state = self.stateStack.getLast();
    ...
    fn tryIdentAsCmd(self: *BaseSexer, token: Token, text: []const u8) ?u16 {
        _ = token;
        const state = self.stateStack.getLast();
...
// Sexer (sexp rewriter)
//
// Optional wrapper supplied by the @lang module. When `mumps.zig` exports a
...
pub const Sexer = if (@hasDecl(mumps, "Sexer")) mumps.Sexer else BaseSexer;
...
pub fn parseCommands(allocator: std.mem.Allocator, source: []const u8) !struct { sexer: Sexer, sexp: Sexp } {
    var s = Sexer.init(allocator, source);
    errdefer s.deinit();
    const sexp = try s.parseCommands();
    return .{ .sexer = s, .sexp = sexp };
}
