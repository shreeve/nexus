...
fn executeAction(self: *BaseParser, ruleId: u16, pass: []Sexp) Sexp {
    return switch (ruleId) {
        ...
        11 => self.emptyList(),
        12 => blk: { var out = self.extendList(pass[0]) catch break :blk self.oomNil(); break :blk self.keepList(&out); },
        13 => self.emptyList(),
        14 => self.list(pass),
        15 => self.list(pass),
        16 => self.list(pass),
        17 => pass[1],
        20 => blk: { var out: std.ArrayListUnmanaged(Sexp) = .empty; out.append(self.allocator(), .{ .tag = .@"+" }) catch break :blk self.oomNil(); out.append(self.allocator(), pass[0]) catch break :blk self.oomNil(); out.append(self.allocator(), pass[2]) catch break :blk self.oomNil(); break :blk self.finishList(&out); },
        21 => blk: { var out: std.ArrayListUnmanaged(Sexp) = .empty; out.append(self.allocator(), .{ .tag = .@"-" }) catch break :blk self.oomNil(); out.append(self.allocator(), pass[0]) catch break :blk self.oomNil(); out.append(self.allocator(), pass[2]) catch break :blk self.oomNil(); break :blk self.finishList(&out); },
        22 => pass[0],
...
// Parse table: 42 states x 29 symbols. 0 = error, > 0 = shift or
...
const terminals = [_]u16{ 1, 9, 10, 12, 13, 14, 17, 18, 19, 26, 27, 28 };
