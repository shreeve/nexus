...
pub const BaseSexer = struct {
    ...
    fn executeAction(self: *BaseSexer, ruleId: u16, pass: []Sexp) Sexp {
        return switch (ruleId) {
            ...
            11 => .{ .list = &[_]Sexp{} },
            12 => blk: { var out: std.ArrayListUnmanaged(Sexp) = .empty; if (pass[0] == .list) for (pass[0].list) |item| out.append(self.allocator(), item) catch break :blk .nil; while (out.items.len > 0 and out.items[out.items.len - 1] == .nil) _ = out.pop(); break :blk .{ .list = out.toOwnedSlice(self.allocator()) catch &[_]Sexp{} }; },
            13 => .{ .list = &[_]Sexp{} },
            14 => self.list(pass),
            15 => self.list(pass),
            16 => self.list(pass),
...
// Sexer (sexp rewriter)
//
// Optional wrapper supplied by the @lang module. When `features.zig` exports a
...
pub const Sexer = if (@hasDecl(features, "Sexer")) features.Sexer else BaseSexer;
