...
test "facts export" {
    ...
    defer p.deinit();
    const tree = try p.parse(.prog);
    // Ids follow construction order: set = 1, the stmts plumbing list = 2
    // (spread into prog, so not in the tree), prog = 3.
    try testing.expectEqualStrings(
        \\(node 3 prog 0 7)
        \\(role 3 stmts 1 leaf 6 1)
        \\(node 1 set 0 5)
