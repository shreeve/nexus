...
pub const BaseSexer = struct {
    ...
    fn executeAction(self: *BaseSexer, ruleId: u16, pass: []Sexp) Sexp {
        return switch (ruleId) {
            ...
            81 => self.sexpSpread(.@"group", pass[1]),
            82 => pass[1],
    ...
    pub fn parseGrammar(self: *BaseSexer) !Sexp {
...
// Sexer (sexp rewriter)
//
// Optional wrapper supplied by the @lang module. When `lang.zig` exports a
...
/// grammar. Caller owns `result.sexer` and must call `result.sexer.deinit()`
/// when done with the returned tree (the tree references arena-allocated
/// memory owned by the sexer).
pub fn parseGrammar(allocator: std.mem.Allocator, source: []const u8) !struct { sexer: Sexer, sexp: Sexp } {
    var s = Sexer.init(allocator, source);
    errdefer s.deinit();
    const sexp = try s.parseGrammar();
